...
//        Prebid.shared.storedAuctionResponse = @"1001-rubicon-300x250";
        ...
        if([adServerName isEqualToString:kDFPString]){
                if ([host isEqualToString:kAppNexusString]) {
                    [Prebid.shared setCustomPrebidServerWithUrl:@"https://ib.adnxs.com/openrtb2/prebid" error:nil];
                } else if ([host isEqualToString:kRubiconString]) {
                    [Prebid.shared setCustomPrebidServerWithUrl:@"https://prebid-server.rubiconproject.com/openrtb2/auction" error:nil];
                }
        }
